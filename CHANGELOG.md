# Changelog

All notable changes for each version are listed under the corresponding `v*.*.*` tag.
Tags and this file are updated by the post-commit hook when you commit with `--patch`, `--minor`, or `--major` (see CONTRIBUTING.md).
You can also run `bun run release:changelog` to rewrite this file from tags.

## [v11.5.0] - 2026-08-24

- feat: support OpenCode device authorization codes (2bdd6fc)

## [v11.4.9] - 2026-08-24

- fix: normalize repeated web form arguments (ef34216)

## [v11.4.8] - 2026-08-24

- docs: note preserving Caddy certificate state (b02d343)

## [v11.4.7] - 2026-08-22

- fix: setup opencode config should have a retry button if /api/setup/opencode/auth (45s timeout) fails (85c4fd9)

## [v11.4.6] - 2026-08-22

- fix: trying to simplify opencode port forwarding (824d124)

## [v11.4.5] - 2026-08-21

- fix: opencode timeout is 30s now, except EADDRINUSE (8f13a00)

## [v11.4.4] - 2026-08-21

- fix: added logs for failing script (12ba415)

## [v11.4.3] - 2026-08-21

- fix: web setup page piper install indicator (6055a1e)

## [v11.4.2] - 2026-08-21

- fix: source profile to run bun (6e9fb1a)

## [v11.4.1] - 2026-08-21

- fix: alpine OS setup script on git dependency and caddy setup (283fae8)

## [v11.4.0] - 2026-08-21

- feat: alpine OS setup script (b693618)

## [v11.3.1] - 2026-08-21

- fix: small wording fixes on version mismatch when the app coreApiVersion is less than current AppWeaver version (9a70dfa)

## [v11.3.0] - 2026-08-21

- feat: run piper as a service option added (bd4b900)

## [v11.2.10] - 2026-08-21

- fix: Cannot find module '../generated/plugins' when no plugin is installed (6df763f)

## [v11.2.9] - 2026-08-21

- fix: soften coreApiVersion restriction on lower version (14b9086)

## [v11.2.8] - 2026-08-21

- fix: removed unnecessary quite flag (ca71e37)

## [v11.2.7] - 2026-08-21

- fix: frontend setup page piper binary bug (9409cc7)

## [v11.2.6] - 2026-08-21

- fix: piper binary now accepts "python3 -m piper" (e72eee5)

## [v11.2.5] - 2026-08-20

- fix: nak is an optional dependency --patch (8442bb7)
- fix: setup redirect bug (017d211)

## [v11.2.4] - 2026-08-20

- feat: AI intervention and session diff tools --minor (26812a3)
- feat: add authenticated OpenAI-compatible inference API --minor (ca0c700)
- feat: capability labeling fix with NIP-32 and plugin release management improvements --minor (31f8b2c)
- feat: new gallery section --minor (b5e5908)
- docs: new bottomup design document --patch (ba5b274)
- feat: milestone 1 for plugins releases changes --minor (087338a)
- fix: some of the plugins workflow has been fixed --patch (93e82a3)
- feat: opencode added support for `question.asked` message and the question/prompt widget --minor (ac93890)
- fix: refactor roadmap as a library to be used by apps/landing and src/commands/roadmap which fixes the issue loading problem in src/commands/roadmap --patch (71d4a6e)
- feat: many nostr post element fixes, added highlight support --minor (1d8bdff)
- fix: added --host argument to run-start script --patch (bcd74bc)
- fix: Production no longer sets BOT_SETUP_UI_ORIGIN to localhost (d1d56ab)

## [v11.2.3] - 2026-08-12

- chore: update packages (4ccae42)

## [v11.2.2] - 2026-08-12

- fix: apply NIP-B7 blossom URL resolution (028c109)

## [v11.2.1] - 2026-08-12

- fix: support legacy generated plugin registrations (e305742)

## [v11.2.0] - 2026-08-12

- feat: add configurable workspace AI instructions (87fbd3d)

## [v11.1.3] - 2026-08-11

- fix: changed from switch button to checkbox in skill manager dialog (dc90806)

## [v11.1.2] - 2026-08-11

- fix: small nostr fixes (3ae156b)

## [v11.1.1] - 2026-08-11

- fix: lint (1af81b4)

## [v11.1.0] - 2026-08-11

- feat: skill manager (4c2ba9f)

## [v11.0.3] - 2026-08-04

- fix: make textarea resizable (ba3bae3)

## [v11.0.2] - 2026-08-04

- fix: web push (3d205ae)

## [v11.0.1] - 2026-08-04

- nit: added --depth=1 to git clone commands (1ae93fe)

## [v11.0.0] - 2026-08-02

- feat: expand WebNode interactions and profile workflows (96f8325)

## [v10.1.2] - 2026-08-02

- chore: add Nostr publish diagnostics (e64025b)

## [v10.1.1] - 2026-08-02

- fix: improve X link previews (29605e8)

## [v10.1.0] - 2026-07-30

- feat: add capability services and optimistic web actions (f952afc)

## [v10.0.1] - 2026-07-23

- fix: scope landing roadmap event loading (324ce15)

## [v10.0.0] - 2026-07-23

- feat: add keyed WebNode reconciliation (a03a1cf)

## [v9.55.0] - 2026-07-23

- feat: integrate Nostr interactions with event resolution (b709f97)

## [v9.54.1] - 2026-07-23

- docs: refresh bottom-up workspace guides (a99281c)

## [v9.54.0] - 2026-07-22

- feat: add cached Nostr event resolution (f9f8a11)

## [v9.53.6] - 2026-07-22

- fix: harden web previews and widget assets (813cf3d)

## [v9.53.5] - 2026-07-15

- chore: ignore local Caddy configuration (eb0000b)

## [v9.53.4] - 2026-07-15

- chore: remove deprecated TypeScript base URL (2c9ff4a)

## [v9.53.3] - 2026-07-15

- fix: serve crawlable app pages and refresh content (d6c1108)

## [v9.53.2] - 2026-07-15

- fix: simplify web viewport and manifest handling (0233464)

## [v9.53.1] - 2026-07-15

- fix: improve relay defaults and DM subscriptions (3cf9f61)

## [v9.53.0] - 2026-07-15

- feat: expand Nostr post interactions (697d214)

## [v9.52.0] - 2026-07-08

- feat: generate static plugin pages with SEO meta tags (0ff1d8e)

## [v9.51.13] - 2026-07-08

- fix: add nostr-radar to landing routes (de5c351)

## [v9.51.12] - 2026-07-08

- fix: landing page hero apps icon fix for nr (77f81e0)

## [v9.51.11] - 2026-07-08

- build: final demo refresh (e65a215)

## [v9.51.10] - 2026-07-08

- build: refresh demo and landing source (966a026)

## [v9.51.9] - 2026-07-08

- style: add nostr radar to hero animation with reduced-motion support (660c415)

## [v9.51.8] - 2026-07-08

- refactor: use topbar-actions-left container for right-placement widgets (4bf53d1)

## [v9.51.7] - 2026-07-08

- build: finalize demo assets (df342c7)

## [v9.51.6] - 2026-07-08

- build: refresh demo with nr widget (3865835)

## [v9.51.5] - 2026-07-08

- fix: right widgets on left, header widgets on right, proper icon path (e9cc130)

## [v9.51.4] - 2026-07-08

- chore: cleanup stale nr icon file (1af9bad)

## [v9.51.3] - 2026-07-08

- chore: include nr icon for landing grid (30b59f2)

## [v9.51.2] - 2026-07-08

- fix: correct nr icon path for landing grid (b0c7887)

## [v9.51.1] - 2026-07-08

- build: update demo and landing assets (a7b0e4d)

## [v9.51.0] - 2026-07-08

- feat: add right-side header widget placement for Nostr Radar (175cb32)

## [v9.50.1] - 2026-07-08

- fix: restore missing apps and add nostr radar branding (a503fc0)

## [v9.50.0] - 2026-07-08

- feat: add nostr radar landing page (d1dda25)

## [v9.49.1] - 2026-07-07

- fix: update web icons and host binding (492509c)

## [v9.49.0] - 2026-07-07

- feat: add nostr web primitives and wot services (ab98ddd)

## [v9.48.0] - 2026-07-01

- feat: support structured web command parsing (a28ed41)

## [v9.47.6] - 2026-07-01

- fix: improve PWA push notifications debugging (199a9f6)

## [v9.47.5] - 2026-07-01

- fix: apps/landing SEO improvements (0e864e5)

## [v9.47.4] - 2026-06-30

- fix: added footer support for nostr posts (7388859)

## [v9.47.3] - 2026-06-28

- blog: new post: "Why build an AppWeaver app?" (a8cb94b)

## [v9.47.2] - 2026-06-28

- fix: blog engine fixes, type fixes (5055847)

## [v9.47.1] - 2026-06-28

- fix: changed apps route to apps/bookmark-manager and so on (7e75c4d)

## [v9.47.0] - 2026-06-28

- feat: new nostr supported blog engine for landing app (ec6d8df)

## [v9.46.3] - 2026-06-28

- fix: fetch zapped profile with query relays (744e2bb)

## [v9.46.2] - 2026-06-27

- fix: moved zap count into issue summary, fixed filter (90bba8a)

## [v9.46.1] - 2026-06-26

- docs: update roadmap and blog drafts (a58352e)

## [v9.46.0] - 2026-06-26

- feat: resolve direct nostr plugin install targets (e89f620)

## [v9.45.1] - 2026-06-26

- feat: align landing roadmap relay discovery (a8a7e98)

## [v9.45.0] - 2026-06-26

- feat: add roadmap board creation flow (0231974)

## [v9.44.0] - 2026-06-26

- feat: resolve roadmap relays from nostr repo addresses (9382440)

## [v9.43.2] - 2026-06-18

- fix: added tree item spacer back (ee4311d)

## [v9.43.1] - 2026-06-17

- fix: subscribe before publishing bunker requests (7697080)

## [v9.43.0] - 2026-06-17

- feat: add relay-backed roadmap panels (92e6db0)

## [v9.42.2] - 2026-06-17

- refactor: share nostr author identity helpers (303acee)

## [v9.42.1] - 2026-06-14

- docs: add story system blog draft (88845ae)

## [v9.42.0] - 2026-06-14

- feat: add landing demo recorder and showcase UI (bd7030a)

## [v9.41.0] - 2026-06-14

- feat: improve story playback controls and targets (0124f07)

## [v9.40.4] - 2026-06-09

- fix: todo duel story (cf88f75)

## [v9.40.3] - 2026-06-09

- fix: working indicator height problem (857bbbd)

## [v9.40.2] - 2026-06-09

- local first blog post (de47166)

## [v9.40.1] - 2026-06-09

- build: deploy landing page (0402557)

## [v9.40.0] - 2026-06-09

- feat: add Cashu melt flow and history kinds (16e4e0a)

## [v9.39.0] - 2026-06-08

- feat: add Routstr provider index and discovery (c424141)

## [v9.38.0] - 2026-06-07

- feat: add Cashu wallet setup and web wallet modal (fbe6f13)

## [v9.37.8] - 2026-06-05

- fix: persist interrupted chat status (6596adb)

## [v9.37.7] - 2026-06-05

- refactor: split WebNodeRenderer into focused modules (4aa4eb0)
- fix: enter on mobile doesn't submit form (fc5eb61)

## [v9.37.6] - 2026-06-05

- ref: xxx (b5f0e59)

## [v9.37.5] - 2026-06-05

- fix: tone status update trigger (b9c2aea)

## [v9.37.4] - 2026-06-05

- fix: segment streamed reasoning cards (24829b2)

## [v9.37.3] - 2026-06-05

- fix: improve opencode stream UI handling (39e7adb)

## [v9.37.2] - 2026-06-05

- fix: no need for open in timeline button in the toolbar (e2de03c)

## [v9.37.1] - 2026-06-04

- fix: update blog todo (f41dd62)

## [v9.37.0] - 2026-06-04

- feat: show plugin updates blocked by core version (76447f5)

## [v9.36.1] - 2026-06-04

- fix: expand tree items before reveal actions (7c4a77f)

## [v9.36.0] - 2026-06-04

- feat: add file web editing primitives (03ca937)

## [v9.35.1] - 2026-06-03

- fix: improve prompt cleanup and chat diagnostics (ce2eaa1)

## [v9.35.0] - 2026-06-03

- feat: add nostr tooling setup recommendations (d4baf91)

## [v9.34.0] - 2026-06-03

- feat: add opencode session context actions (cf75e01)

## [v9.33.3] - 2026-06-03

- added new blog posts (fac3eec)

## [v9.33.2] - 2026-06-01

- ref: added more debug logs to web app with localstorage support (c27eb2f)

## [v9.33.1] - 2026-06-01

- ref: added more debug logs for prompt issues (3bf6646)

## [v9.33.0] - 2026-06-01

- feat: get core changelog (12fb494)

## [v9.32.3] - 2026-06-01

- fix: update-check would paint bot status button to green (a254eb2)

## [v9.32.2] - 2026-06-01

- fix: show changelog button (779e4a4)

## [v9.32.1] - 2026-06-01

- fix: don't give update option if already up-to-date (7b73afe)

## [v9.32.0] - 2026-06-01

- feat: bot update via git (3d5c5ef)

## [v9.31.3] - 2026-06-01

- fix: trying to catch rate limit errors (32e0a6d)

## [v9.31.2] - 2026-05-31

- fix: sometimes composer doesn't add to timeline (afbe849)

## [v9.31.1] - 2026-05-31

- not needed git skill (a5f6b27)

## [v9.31.0] - 2026-05-31

- feat: add core update check indicator (7ad43b5)

## [v9.30.3] - 2026-05-31

- fix: tame opencode sdk server CPU usage (410ed64)

## [v9.30.2] - 2026-05-31

- fix: split timeline thinking card from assistant card (5818a34)

## [v9.30.1] - 2026-05-31

- fix: the story engine reload bug (abcff0d)

## [v9.30.0] - 2026-05-30

- added changelogs to plugins intall (d4ccf58)

## [v9.29.0] - 2026-05-29

- added bot restart to the account top menu (4b0032d)

## [v9.28.1] - 2026-05-28

- fix: preserve dock widgets across timeline updates (7c04205)

## [v9.28.0] - 2026-05-28

- feat: add plugin update flow (be63683)

## [v9.27.8] - 2026-05-28

- fix: standardize web host port env vars (ca90e40)

## [v9.27.7] - 2026-05-28

- fix: improve demo widget playback tracking (52c8098)

## [v9.27.6] - 2026-05-28

- fix: configure local web server startup (e13d49b)

## [v9.27.5] - 2026-05-28

- feat: add plugin release publishing commands (9330e42)

## [v9.27.4] - 2026-05-27

- fix: improve setup and palette input handling (ffa4fa7)

## [v9.27.3] - 2026-05-27

- fix: center landing intro header (ec1fbbf)

## [v9.27.2] - 2026-05-27

- fix: restore account menu and hide demo widgets (cd21cf9)

## [v9.27.1] - 2026-05-27

- feat: refresh landing brand presentation (31f1a16)

## [v9.27.0] - 2026-05-27

- feat: improve demo playback and header chrome (3abd90f)

## [v9.26.0] - 2026-05-26

- feat: polish landing demo flow (ddfd3d1)

## [v9.25.1] - 2026-05-26

- fix: scroll highlighted refresh targets (efb1279)

## [v9.25.0] - 2026-05-24

- feat: polish plugin landing pages (8d9e758)

## [v9.24.0] - 2026-05-23

- feat: add plugin landing demos (d92187d)

## [v9.23.1] - 2026-05-23

- fix: suppress demo programmatic focus (ad5eaad)

## [v9.23.0] - 2026-05-22

- feat: add fixture-backed landing demo (7c57bb0)

## [v9.22.1] - 2026-05-21

- fix: expand landing demo timeline (bfb413c)

## [v9.22.0] - 2026-05-21

- feat: refresh landing copy and demo assets (9043b4f)

## [v9.21.4] - 2026-05-21

- fix: improve setup web startup links (c48d642)

## [v9.21.3] - 2026-05-21

- fix: style widget help open state (6fd484a)

## [v9.21.2] - 2026-05-20

- fix: highlight first-run setup URL (268b8a0)

## [v9.21.1] - 2026-05-20

- fix: polish setup dependency UI (3499870)

## [v9.21.0] - 2026-05-20

- feat: add setup dependency checks (22883d7)

## [v9.20.2] - 2026-05-20

- fix: restore setup page scrolling (9bf51d2)

## [v9.20.1] - 2026-05-20

- fix: improve plugin install feedback (3b759a5)

## [v9.20.0] - 2026-05-20

- feat: add plugin install metadata support (1110dbb)

## [v9.19.0] - 2026-05-20

- feat: publish plugin title metadata (cad1889)

## [v9.18.1] - 2026-05-19

- fix: style of certain buttons (169418c)

## [v9.18.0] - 2026-05-19

- feat: add nostr search relay web support (6fa1b35)

## [v9.17.1] - 2026-05-17

- fix:rename dm-bot to appweaver (6f448a3)

## [v9.17.0] - 2026-05-17

- feat: improve bunker signing UX (9b53fd3)

## [v9.16.0] - 2026-05-16

- feat: copy current diff line references (9772743)

## [v9.15.4] - 2026-05-16

- docs: clarify journal publish wording (f95564a)

## [v9.15.3] - 2026-05-16

- docs: update plugin branding names (5684b1a)

## [v9.15.2] - 2026-05-16

- docs: add Captain's Log plan (0d0c3f2)

## [v9.15.1] - 2026-05-16

- fix: refine web dock header spacing (b613fb0)

## [v9.15.0] - 2026-05-16

- feat: add native session adoption (e634fd3)

## [v9.14.1] - 2026-05-16

- fix: support richer WebNode interactions (a06508c)

## [v9.14.0] - 2026-05-16

- feat: add generic Nostr publish action (01f41d0)

## [v9.13.0] - 2026-05-16

- feat: add widget help stories UI (61fe9ce)

## [v9.12.0] - 2026-05-15

- feat: support persistent web UI reveal state (26b6150)

## [v9.11.0] - 2026-05-14

- feat: add desktop widget dock layout controls (57d04db)

## [v9.10.7] - 2026-05-13

- fix: preparing branding (0e15d6c)

## [v9.10.6] - 2026-05-13

- fix: agent stream git diff summary (935a0ee)

## [v9.10.5] - 2026-05-13

- fix: branding related changes (7316da0)

## [v9.10.4] - 2026-05-13

- fix: opencode scroll fix, added todo tool card (e3cfcd6)

## [v9.10.3] - 2026-05-13

- fix: agent behavour and apply_patch (3e3ef87)

## [v9.10.2] - 2026-05-13

- fix: for todo copy to clipboard (9c665ef)

## [v9.10.1] - 2026-05-13

- fix: improve landing demo playback controls (8889ea4)

## [v9.10.0] - 2026-05-12

- feat: add WebNode scroll on mount support (051a54a)

## [v9.9.16] - 2026-05-12

- docs: clarify plugin git status (e5cf935)

## [v9.9.15] - 2026-05-12

- refactor opencode parts parser (bfb90f6)

## [v9.9.14] - 2026-05-12

- eslint advice (6be541e)

## [v9.9.13] - 2026-05-12

- added dhalsim to nip05 and lud16 (af2fb9d)

## [v9.9.12] - 2026-05-12

- fix: improve opencode stream patch handling (2a4ef0b)

## [v9.9.11] - 2026-05-12

- fix: complete AppWeaver rebrand cleanup (f199b93)

## [v9.9.10] - 2026-05-11

- fix: improve web chat controls (47bc4e5)

## [v9.9.9] - 2026-05-11

- fix: align setup and opencode model state (7083617)

## [v9.9.8] - 2026-05-11

- fix: improve docker setup runtime (736652e)

## [v9.9.7] - 2026-05-11

- fix: use workspace root for opencode agents (da3fec4)

## [v9.9.6] - 2026-05-11

- fix: run docker workspace as non-root (6a25554)

## [v9.9.5] - 2026-05-11

- fix: persist opencode setup auth state (56c871d)

## [v9.9.4] - 2026-05-11

- fix: add dbus for docker vnc session (03bc2a9)

## [v9.9.3] - 2026-05-11

- fix: detect opencode oauth setup status (b930aa7)

## [v9.9.2] - 2026-05-11

- fix: show opencode auth completion (a2207c5)

## [v9.9.1] - 2026-05-11

- fix: rebuild web UI on startup (fd83f0c)

## [v9.9.0] - 2026-05-11

- feat: add timeline event command outputs (152a29d)

## [v9.8.0] - 2026-05-11

- feat: add setup web configuration flow (248d0a3)

## [v9.7.0] - 2026-05-09

- feat: add docker setup flow (aab6b20)

## [v9.6.4] - 2026-05-09

- fix: skip startup wallet state hydration (6906d97)

## [v9.6.3] - 2026-05-09

- fix: render modal timeline UI outputs (a10e781)

## [v9.6.2] - 2026-05-08

- fix: preserve narrowed story widget targets (0f0acef)

## [v9.6.1] - 2026-05-08

- fix: restore web dist SPA fallback path (5a88683)

## [v9.6.0] - 2026-05-08

- feat: add interactive roadmap issue actions (778b524)

## [v9.5.2] - 2026-05-08

- fix: improve plugin catalog publishing and queries (9dc1a9d)

## [v9.5.1] - 2026-05-08

- fix: point status agent actions at nested command (26f7e75)

## [v9.5.0] - 2026-05-08

- feat: sync wallet state over nostr (2f8f74f)

## [v9.4.0] - 2026-05-08

- feat: nest AI agent management commands (a554243)

## [v9.3.4] - 2026-05-08

- fix: hide piper TTS control when unavailable (7e88c72)

## [v9.3.3] - 2026-05-08

- demo app doesn't support piper TTS (d144d7d)

## [v9.3.2] - 2026-05-08

- serve nip05 and lud16 for appweaver account (a16ec05)

## [v9.3.1] - 2026-05-06

- added vercel api function to serve nostr.json in apps/landing (341ef5d)

## [v9.3.0] - 2026-05-06

- roadmap and plugin management (6dba4ef)

## [v9.2.4] - 2026-05-03

- bun run start should start the UI too (6a0f4ac)

## [v9.2.3] - 2026-05-03

- trying bun run start, 5551 asset 404 error fix (804797d)

## [v9.2.2] - 2026-05-03

- fix pwd on windows (5fb877d)

## [v9.2.1] - 2026-05-03

- fix state table not exist error (b7daf3a)

## [v9.2.0] - 2026-05-03

- demo improvements, and other improvements (af40292)

## [v9.1.0] - 2026-05-01

- demo system (8bd4b3f)

## [v9.0.1] - 2026-04-16

- landing page and improved PWA web page (f8350bb)

## [v9.0.0] - 2026-04-15

- AppWeaver branding. New web interface (edc304c)

## [v8.1.1] - 2026-03-31

- get rid of env in the backend creation (4f69cdc)

## [v8.1.0] - 2026-03-29

- bunker add list commands (c91d5da)

## [v8.0.0] - 2026-03-28

- wot (1d78689)

## [v7.9.0] - 2026-03-28

- redraw fixes and checkpoint for wot (2c49cc4)

## [v7.8.1] - 2026-03-27

- fixes on publish scripts (2a4a084)

## [v7.8.0] - 2026-03-27

- publish skill and scripts (2378ec8)

## [v7.7.0] - 2026-03-23

- new prompt functionality for plugin interactive loops (045715b)

## [v7.6.0] - 2026-03-23

- Initial commit: Add dm-bot NIP-17 agent, project rules, and supporting files (741cc78)
- Update bot configuration and documentation (2b0cda0)
- Refactor code style and improve documentation (7f83038)
- Enhance message chunking and error handling in DM publishing (6a9be7b)
- add UNLICENSE LICENSE file (80f5b66)
- Add !ask shortcut for mode switching. (43b8694)
- Update README and agent-cli-permission rules for npm script whitelisting (2df04f7)
- Update watch mode functionality and documentation (90be934)
- Add local/remote reply transport commands and local-only bot mode. (9734e6b)
- Implement workspace targeting and post-agent linting functionality (19b39a4)
- Add publish-10050.sh script for DM relay publish (7f02e04)
- new backend opencode (4f1d4c7)
- fix (4096912)
- big refactor (7046162)
- fixed a minor bug (c1c667e)
- fixed a minor bug (e6b5034)
- new commit hook that bumps package.json version using semver rules --minor (fcf4b2d)
- docs: add contributing guide --minor (3af210d)
- fix version bump was after commit --patch (84c99f4)
- fix again (2c9af97)
- try again (f0d8ce0)
- test commit (4d88bb4)
- fix: another test (version was 1.2.4) (7bea7e0)
- fix: try again (version was 1.2.6 now set back to 1.0.0) (d969269)
- fix: try again (version was 1.0.0 and not staged, I expect to see 1.0.1 after commit) (e1d9441)
- chore: replace prepare-commit-msg with commit-msg and post-commit hooks (255a4a2)
- cashu wallet is added (7827030)
- test version bump (c6444f3)
- checkpoint on integration (c2e76fc)
- fix Cashu wallet bugs. Added new setup scripts for wallet and nostr configuration (3b44ab9)
- improved logging, wallet setup, error handling with retries for token reception. (875d2f0)
- fix: added fail over forgotton commit version postfix (c1ea8a4)
- forgot a semicolon (887373f)
- better logging in wallet operations (7df9c68)
- checkpoint (f8dd146)
- refactor checkpoint (1b50fcc)
- fixes (135feb2)
- refactor index.ts, prevent flash crashes when running with run-with-restart (ac08334)
- Add OpenCode SDK backend and new bot commands (!lint off, !log info off, !ready off) (b805c75)
- added warn messages when provider has changed and the model is incompatible (a724e09)
- new command to add models to the opencode.json from provider/routstr models (f8f2bf2)
- fixed opencode-sdk + routstr prompting by copying the .env variables into the process (65024e2)
- cleaning (ee70179)
- updated readme with installation instructions (8eb23e2)
- Enhance README with Cursor and Routstr integration details - (ad057d1)
- main repo (dc8c4ae)
- added links to readme (3b69be8)
- add task scheduling functionality (9837889)
- publishing kind 0 event for the bot in the nostr-setup script (6c96453)
- fixed accidental ansi leakage to the DMs (4601ea3)
- Update workspace command usage messages to include dynamic options (0b7b23f)
- Add contributing guidelines to the AI rules with a reference to the CONTRIBUTING.md file (b050041)
- added missing dependencies and prettier config (8b68539)
- nits --patch (9cffa0e)
- Remove test.txt (67f2541)
- improved contributing rules (60b1aa0)
- test (31cb048)
- why is the version flag not enforced (40010ea)
- yo (6eaa86e)
- fix post commit (be5953a)
- chore: remove test.txt --patch (7649d35)
- chore: CLI test for bun hooks (3d62d81)
- git scripts are now bun (d32446d)
- chore: remove test.txt (f552d02)
- fix: task current time & timezone (78ba355)
- test push (7513ecd)
- test: add file-sync (7bffee5)
- test: add file-sync spec (32c8da3)
- feat: sync file between bot instances --minor (ee269e3)
- potential bug fix --patch (30ad857)
- fix file paths --patch (bba04e9)
- test: add bun.lockb (056e3d0)
- lint (541913c)
- added `!bot npub` command (5787aaa)
- feat: add !bot restart command and enhance lint functionality: `!lint` manually runs the lint by the bot (136ee61)
- chore: update contributing guidelines and add ngit-helper script for improved Git workflow (6ac1643)
- checkpoint (f5a84b2)
- refactor: update todos table schema to use INTEGER for IDs and adjust related types in TypeScript (031a7e9)
- feat: todos schema, commands and formatting (e821f2d)
- feat: add schedule_description to task schema and related functions (53897de)
- feat: implement todo AI command and enhance todo tools (bc09a0b)
- fix (6451dbe)
- feat: status reporting, workspace PWD (1b9af1f)
- enhance new session command with status reporting (f2ebb0f)
- finilize todos (f51eb31)
- cleanup (be02734)
- renamed tasks to jobs (2e970ff)
- new docs for future features (5706ebf)
- Put the Todo Draft ID in the outputs and descriptions of the prompt (26d3c9b)
- feat: introduce job AI command for natural language job creation and implement draft storage (96d5291)
- Use z.infer to derive TS types from Zod schemas (1067abe)
- refactor: reorganize todo-ai command structure and enhance draft handling (b3a8919)
- trying to fix opencode-sdk no response bug (4be2da8)
- fix: add model override logging and ensure correct input structure for session prompts in opencode-sdk (ec9e5eb)
- workaround of parent opencode-sdk bug (0130b2f)
- fixed a bug with the workspace folder calculation, replaced npm run with bun run in the docs, added workspace_target to jobs table (5571994)
- cleanup, index.ts refactor (c8f209b)
- fix cwd bugs and unnecessary session creation in todos and jobs (0526fff)
- new plugin system, migrated todos (e35201f)
- improved bunker in the publish plugin script (2e12558)
- fix failure when plugins.json does not exist (f309c67)
- adding repo url to plugin event (17cb867)
- fix: proper install-plugin script (df21561)
- fix plugin:generate (8dd2daa)
- fix generate tools plugin path by alias (226cfcd)
- fix delete is not allowed as a variable declaration name in tool generation (67eb5d7)
- fix: fail when no plugins installed and generated folder is not generated yet (d1f7e55)
- removed hard coded tool names (86b827b)
- fix: disable default linting setting (6190b44)
- refactor: enhance install-plugin script to support updating plugins and improve repo identity resolution (e72c5fd)
- new documentation on PLUGINS, new script for bot-setup (87c7434)
- refactor: update bot setup script to manage .gitignore entries and improve symlink handling (c463f35)
- big refactor, new plugin system (cc464d9)
- removed core jobs, and fixes (9c811e4)
- feat: add optional description field to plugin schema and enhance event content handling (3de7ad8)
- nits (996e49e)
- small refactor (8c04144)
- lint (67decbb)
- bot setup script, for cursor backend ask for CURSOR_API_KEY (70c6916)
- updated plugins.md (3a396be)
- new plugin template and script (d360a36)
- lint (bd273d5)
- lint (c96737c)
- lint (0187f7b)
- plugin:new runs lint after creating the files. changed plugin template extension from .ts to .ts.template (5a021e0)
- New plugin tooling via CLI and SKILLS files. (0d1edd1)
- install plugin version checks (310d797)
- fix (c09149e)
- bot setup script to create symlinks for skill folders in parent project (0e11272)
- send dm skill added (0403051)
- docs fix (4f08459)
- workspace-tree skill (337eb62)
- some fixes, moved file to its own plugin (53a704b)
- documents (77fd197)
- fix DB lock issue with WAL mode (9f15a25)
- added bunker management script (ac54f2d)
- bunker script (6f0837d)
- Enhance contribution guidelines and scripts for versioning and changelog management. Added support for plugin repositories and improved commit message handling in hooks. (32d63c8)
