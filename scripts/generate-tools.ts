...
const pluginsRegistrations = pluginsJson.plugins
  .map(
    (e) =>
      `  registerPlugin({ alias: '${e.alias}', plugin: ${toPluginExportName(e.alias)}, ctx });`,
  )
  .join('\n');
...
import type { PluginHostContext } from '../src/core/plugin';
import { registerPlugin } from '../src/core/registry';
${pluginsImports}

export function registerPlugins(ctx: PluginHostContext): void {
