...
export function renderSessionAttachText(
  ...
  switch (d.view) {
    case 'usage':
      return `Usage: ${d.prefix}session attach opencode <session_id>`;
    case 'backend-mismatch':
      return `Cannot attach an OpenCode session while the agent backend is ${d.activeBackend} — prompts would go to the wrong runtime. Set backend first, e.g. ${d.prefix}ai backend opencode, then run attach again.`;
    case 'success':
