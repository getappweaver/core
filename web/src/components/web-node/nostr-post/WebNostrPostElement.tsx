...
import {
  getNostrInteractionFlags,
  nostrArchivedOverride,
} from '../../../nostr/interactionState';
import {
...
import { NostrPostReplies } from './NostrPostReplies';
...
function referenceActionItems(
  ...
  const archived =
    nostrArchivedOverride(reference.id) ?? reference.archived === true;
...
export function WebNostrPostElement(
  ...
  const actionItems = createMemo<PostActionItem[]>(() => {
    ...
    const liked = post?.nostrLiked === true || flags.liked;
    const replied = post?.nostrReplied === true || flags.replied;
    const reposted = post?.nostrReposted === true || flags.reposted;
    const quoted = post?.nostrQuoted === true || flags.quoted;

    const archived =
      nostrArchivedOverride(post?.nostrEventId) ?? post?.nostrArchived === true;

    if (post?.nostrReadAction) {
      items.push({
        label: 'Read',
        action: post.nostrReadAction,
        disabled: false,
        success: false,
      });
    }

    if (post?.nostrArchiveAction) {
      items.push({
        label: archived ? 'Archived' : 'Archive',
        action: post.nostrArchiveAction,
        disabled: false,
        success: archived,
      });
    }

    for (const extraAction of post?.nostrExtraActions ?? []) {
      items.push({
    ...
    if (post?.nostrLikeAction) {
      items.push({
        label: liked
          ? 'Liked'
          : post.nostrLikeCount != null
            ? `Like +${post.nostrLikeCount}`
            : 'Like',
        action: liked ? null : post.nostrLikeAction,
        disabled: liked,
        success: false,
      });
    }

    if (post?.nostrReplyAction) {
      items.push({
        label: replied
          ? 'Replied'
          : post.nostrReplyCount != null
            ? `Reply (${post.nostrReplyCount})`
            : 'Reply',
        action: post.nostrReplyAction,
        disabled: false,
        success: replied,
      });
    }

    if (post?.nostrRepostAction) {
      const label = quoted ? 'Quoted' : reposted ? 'Reposted' : 'Repost';

      items.push({
        label,
        action: reposted || quoted ? null : post.nostrRepostAction,
        disabled: reposted || quoted,
        success: reposted || quoted,
      });
    }
